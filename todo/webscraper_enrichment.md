...
Add a new web-scraping enricher that fits the existing batch enrichment flow and DI/testing conventions already used in `internal/service` and `internal/connector/union/spatialist`. The implementation will isolate external effects (HTTP/time/parsing) behind injected seams, return typed flattened output, and include comprehensive stdlib-only unit tests before wiring into the Union connector surface.
...
[ ] Add an enricher service wrapper (mirroring `SpatialistEnricher` patterns) that reads batch raw records, applies the transform, and aggregates partial failures consistently.
